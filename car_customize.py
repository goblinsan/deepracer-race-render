...
def modifyCarAttributes(texture_path, iterString, car_number, car_color, team_name):
    ...
    # update banner text
    bpy.data.objects['team_name' + iterString].data.body = team_name.split("|")[0]
    bpy.data.objects['city_name' + iterString].data.body = team_name.split("|")[1]
    bpy.data.objects['team_name_depth' + iterString].data.body = team_name.split("|")[0]
