...
def evaluate_and_sort(raw_log_data):
    ...
    df_end_state = df_end_state.sort_values(by=["team", "progress", "lap_time"],
...
def process_team_log_file(team):
    ...
    for line in open_read(log_file_name).splitlines():
...
def generate_leaderboards(race_data):
    summary = race_data[
        ["race_number", "team", "car_no", "lap_end_state", "lap_progress", "lap_time"]].copy().drop_duplicates()
...
def generate_races(race_data):
    ...
    for race_no in race_list:
        ...
        for team in race_teams:
            ...
            race_data_path = path.join("race_data_single_laps", "coord_plots",
                                       f"race {race_no}".replace(" ", "_").lower())
